...
def do_provider_invoke():
    value = invoke("test:index:MyFunction", props={"value": 9000}, opts=InvokeOptions(provider=prov))
    return value["value"]
...
def do_provider_invoke_with_parent(parent):
    value = invoke("test:index:MyFunctionWithParent", props={"value": 41}, opts=InvokeOptions(parent=parent))
    return value["value"]
